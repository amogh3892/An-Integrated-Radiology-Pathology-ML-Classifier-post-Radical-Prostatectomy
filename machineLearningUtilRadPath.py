...
class MachineLearningPipeline(object):
    def __init__(self,featuresPath,labelspath,outputfoldername,config,fs):
        ...
        self.config = config
        self.config['StandardNormalize'] = False 
        self.config['RobustNormalize'] = True 
    ...
    def _Lasso_FS(self,trainfeaturesdf,testfeaturesdf, trainlabels, noFeatures):
        ...
        selector = SelectFromModel(model, max_features=noFeatures)
    ...
    def _CPHELASTIC_FS(self,trainfeaturesdf,testfeaturesdf, trainlabelstime2event,noFeatures):
    ...
    def getCPHPred(self,trainfeatures,trainlabels, testfeatures):
    ...
    def _get_cross_validation_results(self,trainfeaturesdf,trainlabels,traintime2event,cv,runs):
        ...
        for run in range(runs):
            ...
            for train_index, test_index in skf.split(trainfeatures, trainlabels):
                ...
                radselectedfeatnames = [x for x in selectedfeatnames if ("T2" in x) or ("ADC" in x)]
                pathselectedfeatnames = [x for x in selectedfeatnames if not (("T2" in x) or ("ADC" in x))] 

                if config['FeatSelect'] == FS.CPHELASTIC:

                    X_train_rad, X_test_rad, radselectedfeatnames = self._CPHELASTIC_FS(pd.DataFrame(X_train,columns=selectedfeatnames)[radselectedfeatnames],
                                pd.DataFrame(X_test,columns=selectedfeatnames)[radselectedfeatnames],y_train,self.params['noFeatures'][0])

                    X_train_path, X_test_path, pathselectedfeatnames = self._Lasso_FS(pd.DataFrame(X_train,columns=selectedfeatnames)[pathselectedfeatnames],
                                pd.DataFrame(X_test,columns=selectedfeatnames)[pathselectedfeatnames],np.array([x[0] for x in y_train]),self.params['noFeatures'][1])

                    # X_train = np.column_stack((X_train_rad,X_train_path))
                    # X_test = np.column_stack((X_test_rad,X_test_path))

                    selectedfeatnames = radselectedfeatnames + pathselectedfeatnames
                ...
                linear_cca.fit(X_train_rad, X_train_path)

                X_train_rad, X_train_path = linear_cca.transform(X_train_rad,X_train_path)
                X_test_rad,X_test_path = linear_cca.transform(X_test_rad,X_test_path)

                X_train = np.column_stack((X_train_rad,X_train_path))
                X_test = np.column_stack((X_test_rad,X_test_path))
                
                _,pred = self.getCPHPred(X_train,y_train,X_test)

                # radtrainPred, radtestPred = self.getCPHPred(X_train_rad,y_train,X_test_rad)
                # pathtrainPred, pathtestPred = self.getCPHPred(X_train_path,y_train,X_test_path)

                # pred = radtestPred + pathtestPred
                # pred = np.multiply(radtestPred,pathtestPred)
                # _,pred = self.getCPHPred(np.column_stack((radtrainPred,pathtrainPred)),y_train,np.column_stack((radtestPred,pathtestPred)))
